...
async function run() {
  ...
  const prs = data.repository.pullRequests.nodes.filter(pr =>
    ...
    && pr.reviewRequests.nodes.find(requestedReviewers => requestedReviewers.requestedReviewer.name === process.env.REVIEWER)
    && !pr.reviews.nodes.flatMap(review => review.onBehalfOf.nodes).find(behalf => behalf.name === process.env.REVIEWER)
  )
  if (prs.length === 0) {
